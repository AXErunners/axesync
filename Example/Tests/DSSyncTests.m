...
- (void)testInitialHeadersSync2000 {
    if (@available(iOS 13.0, *)) {
        ...
        [self.chain.chainManager.peerManager setTrustedPeerHost:@"178.128.228.195:9999"];
        [[AxeSync sharedSyncController] wipePeerDataForChain:self.chain inContext:[NSManagedObjectContext peerContext]];
        [[AxeSync sharedSyncController] wipeBlockchainDataForChain:self.chain inContext:[NSManagedObjectContext chainContext]];
        [[AxeSync sharedSyncController] wipeSporkDataForChain:self.chain inContext:[NSManagedObjectContext chainContext]];
        [[AxeSync sharedSyncController] wipeMasternodeDataForChain:self.chain inContext:[NSManagedObjectContext chainContext]];
        //
        ...
        [self measureWithMetrics:@[[[XCTCPUMetric alloc] init],[[XCTMemoryMetric alloc] init],[[XCTClockMetric alloc] init]] options:options block:^{
            ...
            [[AxeSync sharedSyncController] startSyncForChain:self.chain];
            ...
            [[NSNotificationCenter defaultCenter] addObserverForName:DSChainInitialHeadersDidFinishSyncingNotification object:nil
                ...
                [[AxeSync sharedSyncController] stopSyncForChain:self.chain];
                [self.chain.chainManager.peerManager removeTrustedPeerHost];
