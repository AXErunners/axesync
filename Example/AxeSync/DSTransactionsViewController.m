...
-(UILabel*)titleLabel {
    ...
    NSMutableAttributedString * attributedAxeString = [[manager attributedStringForAxeAmount:self.chainPeerManager.chain.balance withTintColor:[UIColor whiteColor]] mutableCopy];
    NSString * titleString = [NSString stringWithFormat:@" (%@)",
                              [manager localCurrencyStringForAxeAmount:self.chainPeerManager.chain.balance]];
...
-(void)configureCell:(DSTransactionTableViewCell*)cell atIndexPath:(NSIndexPath*)indexPath {
    ...
    cell.remainingAmountLabel.attributedText = (authenticationManager.didAuthenticate) ? [priceManager attributedStringForAxeAmount:balance withTintColor:cell.remainingAmountLabel.textColor axeSymbolSize:CGSizeMake(9, 9)] : nil;
    ...
    if (sent > 0 && received == sent) {
        cell.amountLabel.attributedText = [priceManager attributedStringForAxeAmount:sent];
        cell.fiatAmountLabel.text = [NSString stringWithFormat:@"(%@)",
                                     [priceManager localCurrencyStringForAxeAmount:sent]];
        cell.directionLabel.text = NSLocalizedString(@"moved", nil);
        cell.directionLabel.textColor = [UIColor blackColor];
    }
    else if (sent > 0) {
        cell.amountLabel.attributedText = [priceManager attributedStringForAxeAmount:received - sent];
        cell.fiatAmountLabel.text = [NSString stringWithFormat:@"(%@)",
                                     [priceManager localCurrencyStringForAxeAmount:received - sent]];
        ...
        cell.amountLabel.attributedText = [priceManager attributedStringForAxeAmount:received];
        cell.fiatAmountLabel.text = [NSString stringWithFormat:@"(%@)",
                                     [priceManager localCurrencyStringForAxeAmount:received]];
