...
- (void)setTransaction:(DSTransaction *)transaction
{
    ...
    for (NSString *address in transaction.outputAddresses) {
        ...
        else if ((account = [transaction.chain accountContainingAxepayExternalDerivationPathAddress:address])) {
    ...
    if (self.sent > 0 && fee > 0 && fee != UINT64_MAX) {
        [text addObject:@""];
        [detail addObject:NSLocalizedString(@"axe network fee", nil)];
            ...
            switch (indexPathRow) {
                ...
                case 6:
                {
                    ...
                    if (roundedFeeCostPerByte != UINT64_MAX) { //otherwise it's being received and can't know.
                        cell.statusLabel.text = roundedFeeCostPerByte == 1?NSLocalizedString(@"1 hak/byte",nil):[NSString stringWithFormat:NSLocalizedString(@"%d haks/byte",nil), (int)roundedFeeCostPerByte];
                    } else {
                ...
                case 7:
                {
                    ...
                    if (self.sent > 0 && self.sent == self.received) {
                        cell.amountLabel.attributedText = [walletManager attributedStringForAxeAmount:self.sent];
                        cell.fiatAmountLabel.text = [NSString stringWithFormat:@"(%@)",
                                                     [walletManager localCurrencyStringForAxeAmount:self.sent]];
                    }
                    else {
                        cell.amountLabel.attributedText = [walletManager attributedStringForAxeAmount:self.received - self.sent];
                        cell.fiatAmountLabel.text = [NSString stringWithFormat:@"(%@)",
                                                     [walletManager localCurrencyStringForAxeAmount:self.received - self.sent]];
                    }
...
#pragma clang diagnostic pop
                        ...
                        cell.amountLabel.attributedText = [walletManager attributedStringForAxeAmount:[self.outputAmount[indexPath.row] longLongValue] withTintColor:cell.amountLabel.textColor axeSymbolSize:CGSizeMake(9, 9)];
                        cell.fiatAmountLabel.text = [NSString stringWithFormat:@"(%@)",
                                                     [walletManager localCurrencyStringForAxeAmount:[self.outputAmount[indexPath.row]
                                                                                                      longLongValue]]];
                    }
                    cell.fiatAmountLabel.textColor = cell.amountLabel.textColor;
                }
...
//                        cell.statusLabel.text = [[DSPriceManager sharedInstance] stringForAxeAmount:blockchainIdentityRegistrationTransaction.topupAmount];
//                        cell.moreInfoLabel.text = nil;
//                        return cell;
//                        break;
//                    }
...
//                        cell.selectionStyle = UITableViewCellSelectionStyleNone;
//                        cell.titleLabel.text = NSLocalizedString(@"topup amount:", nil);
//                        cell.statusLabel.text = [[DSPriceManager sharedInstance] stringForAxeAmount:blockchainIdentityTopupTransaction.topupAmount];
//                        cell.moreInfoLabel.text = nil;
//                        return cell;
//                        break;
//                    }
