...
-(NSPredicate*)predicate {
    return [NSPredicate predicateWithFormat:@"sourceContact == %@ && (SUBQUERY(sourceContact.incomingRequests, $friendRequest, $friendRequest.sourceContact == SELF.destinationContact).@count == 0)",[self.blockchainIdentity matchingAxepayUserInContext:self.context]];
}
...
-(NSPredicate*)invertedPredicate {
    return [NSPredicate predicateWithFormat:@"destinationContact == %@ && (SUBQUERY(destinationContact.outgoingRequests, $friendRequest, $friendRequest.destinationContact == SELF.sourceContact).@count > 0)",[self.blockchainIdentity matchingAxepayUserInContext:self.context]];
}


- (NSArray<NSSortDescriptor *> *)sortDescriptors {
    NSSortDescriptor *usernameSortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"destinationContact.associatedBlockchainIdentity.axepayUsername.stringValue"
                                                                           ascending:YES];
