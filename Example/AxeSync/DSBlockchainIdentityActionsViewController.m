...
- (void)viewDidLoad {
    ...
    if (self.blockchainIdentity.registered && self.blockchainIdentity.currentAxepayUsername && [self.blockchainIdentity statusOfAxepayUsername:self.blockchainIdentity.currentAxepayUsername] == DSBlockchainIdentityUsernameStatus_Confirmed) {
...
-(void)reloadRegistrationInfo {
    ...
    } else if (!self.blockchainIdentity.currentAxepayUsername) {
        self.aboutMeLabel.text = @"Set Username";
        self.usernameStatusLabel.text = @"";
    } else if ([self.blockchainIdentity statusOfAxepayUsername:self.blockchainIdentity.currentAxepayUsername] != DSBlockchainIdentityUsernameStatus_Confirmed) {
        self.aboutMeLabel.text = @"Register Username";
        switch ([self.blockchainIdentity statusOfAxepayUsername:self.blockchainIdentity.currentAxepayUsername]) {
    ...
    } else if (!self.blockchainIdentity.matchingAxepayUserInViewContext.remoteProfileDocumentRevision) {
        self.aboutMeLabel.text = @"Fetching";
        [self.avatarImageView sd_setImageWithURL:nil];
        self.usernameStatusLabel.text = @"";
    }
    else {
        self.aboutMeLabel.text = self.blockchainIdentity.matchingAxepayUserInViewContext.publicMessage;
        [self.avatarImageView sd_setImageWithURL:[NSURL URLWithString:self.blockchainIdentity.matchingAxepayUserInViewContext.avatarPath]];
        self.usernameStatusLabel.text = @"";
    }
}

-(void)loadProfileInitial {
    self.title = self.blockchainIdentity.currentAxepayUsername;
...
-(void)updateProfile {
    self.title = self.blockchainIdentity.currentAxepayUsername;
    if (!self.blockchainIdentity.matchingAxepayUserInViewContext.remoteProfileDocumentRevision) {
        self.aboutMeLabel.text = @"Register Profile";
        [self.avatarImageView sd_setImageWithURL:nil];
    }
    else {
        if (!self.blockchainIdentity.matchingAxepayUserInViewContext.publicMessage) {
...
-(void)reloadContactInfo {
    DSAxepayUserEntity * axepayUser = [self.blockchainIdentity matchingAxepayUserInViewContext];
    DSAxepayUserEntity * activeSentToFriend = [[axepayUser mostActiveFriends:DSAxepayUserEntityFriendActivityType_OutgoingTransactions count:1 ascending:NO] firstObject];
    DSAxepayUserEntity * activeReceivedFromFriend = [[axepayUser mostActiveFriends:DSAxepayUserEntityFriendActivityType_IncomingTransactions count:1 ascending:NO] firstObject];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (indexPath.row == 2) { // About me / Register
            ...
            } else if (self.blockchainIdentity.currentAxepayUsername && [self.blockchainIdentity statusOfAxepayUsername:self.blockchainIdentity.currentAxepayUsername] != DSBlockchainIdentityUsernameStatus_Confirmed) {
    ...
    else if (indexPath.section == 1) { //Axepay
...
//
//            __weak typeof(self) weakSelf = self;
//            [self.chainManager.DAPIClient ds_registerAxePayContractForUser:self.blockchainIdentity forChain:self.chainManager.chain completion:^(NSError * _Nullable error) {
//                __strong typeof(weakSelf) strongSelf = weakSelf;
//                if (!strongSelf) {
//                    return;
//                }
