...
@property (nonatomic,strong) DSKeyValueTableViewCell * axepayContractIDTableViewCell;
...
- (void)viewDidLoad {
    ...
    self.axepayContractIDTableViewCell = [self.tableView dequeueReusableCellWithIdentifier:@"AxepayContractIDCellIdentifier"];
    ...
    if (!self.chain) {
        ...
        self.axedPortTableViewCell.valueTextField.text = [NSString stringWithFormat:@"%u",self.chain.standardPort];
        ...
        self.axepayContractIDTableViewCell.valueTextField.text = !uint256_is_zero(self.chain.axepayContractID)?uint256_base58(self.chain.axepayContractID):@"";
    }
    ...
    switch (indexPath.section) {
        ...
        case 2:
            switch (indexPath.row) {
                ...
                case 1:
                    return self.axepayContractIDTableViewCell;
                default:
                    NSAssert(NO, @"Unknown cell");
                    return [[UITableViewCell alloc] init];
            }
...
-(IBAction)save {
    ...
    UInt256 axepayContractID = [self.axepayContractIDTableViewCell.valueTextField.text isEqualToString:@""]?UINT256_ZERO:[self.axepayContractIDTableViewCell.valueTextField.text base58ToData].UInt256;
    ...
    if (self.chain) {
        [[DSChainsManager sharedInstance] updateDevnetChain:self.chain forServiceLocations:self.insertedIPAddresses withMinimumDifficultyBlocks:minimumDifficultyBlocks standardPort:axedPort dapiJRPCPort:dapiJRPCPort dapiGRPCPort:dapiGRPCPort dpnsContractID:dpnsContractID axepayContractID:axepayContractID protocolVersion:protocolVersion minProtocolVersion:minProtocolVersion sporkAddress:sporkAddress sporkPrivateKey:sporkPrivateKey];
    } else {
        NSString * identifier = self.addDevnetNameTableViewCell.valueTextField.text;
        [[DSChainsManager sharedInstance] registerDevnetChainWithIdentifier:identifier forServiceLocations:self.insertedIPAddresses withMinimumDifficultyBlocks:minimumDifficultyBlocks standardPort:axedPort dapiJRPCPort:dapiJRPCPort dapiGRPCPort:dapiGRPCPort dpnsContractID:dpnsContractID axepayContractID:axepayContractID protocolVersion:protocolVersion minProtocolVersion:minProtocolVersion sporkAddress:sporkAddress sporkPrivateKey:sporkPrivateKey];
    }
