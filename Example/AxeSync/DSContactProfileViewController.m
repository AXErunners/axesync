...
- (TextFieldFormCellModel *)displayNameCellModel {
    if (!_displayNameCellModel) {
        ...
        cellModel.text = self.blockchainIdentity.matchingAxepayUserInViewContext.displayName;
        __weak typeof(self) weakSelf = self;
        cellModel.didReturnValueBlock = ^(TextFieldFormCellModel *_Nonnull cellModel) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) {
...
- (TextFieldFormCellModel *)avatarCellModel {
    if (!_avatarCellModel) {
        ...
        cellModel.placeholder = [NSString stringWithFormat:@"https://api.adorable.io/avatars/120/%@.png",
                                                           self.blockchainIdentity.currentAxepayUsername];
        cellModel.text = self.blockchainIdentity.matchingAxepayUserInViewContext.avatarPath;
        __weak typeof(self) weakSelf = self;
...
- (TextViewFormCellModel *)aboutMeCellModel {
    if (!_aboutMeCellModel) {
        ...
        cellModel.placeholder = [NSString stringWithFormat:@"Hey I'm a demo user %@", self.blockchainIdentity.currentAxepayUsername];
        cellModel.text = self.blockchainIdentity.matchingAxepayUserInViewContext.publicMessage;
...
- (NSArray<BaseFormCellModel *> *)profileItems {
    if (self.blockchainIdentity.currentAxepayUsername) {
...
- (void)updateUsername {
    if (![self.usernameCellModel.text isEqualToString:@""]) {
        [self.blockchainIdentity addAxepayUsername:self.usernameCellModel.text save:YES];
...
- (void)saveButtonAction {
    ...
    // TODO: show HUD
    BOOL isCreate = !self.blockchainIdentity.matchingAxepayUserInViewContext;
    ...
    [self.blockchainIdentity updateAxepayProfileWithDisplayName:displayName publicMessage:aboutMe avatarURLString:avatarURLString];
    __weak typeof(self) weakSelf = self;
