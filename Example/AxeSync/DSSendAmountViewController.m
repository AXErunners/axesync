...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (textField == self.addressField) {
        ...
        if ([novelString isValidAxeAddressOnChain:self.account.wallet.chain]) {
            self.isValidAddress = TRUE;
        } else {
            self.isValidAddress = FALSE;
        }
    } else if (textField == self.amountField) {
        if ([[DSPriceManager sharedInstance] amountForAxeString:novelString] > 0) {
...
-(IBAction)send:(id)sender {
    ...
    paymentRequest.amount = [[DSPriceManager sharedInstance] amountForAxeString:self.amountField.text];
    
    if ([paymentRequest isValidAsNonAxepayPaymentRequest]) {
        ...
        [self.account.wallet.chain.chainManager.transactionManager confirmPaymentRequest:paymentRequest usingUserBlockchainIdentity:nil fromAccount:self.account acceptInternalAddress:YES acceptReusingAddress:YES addressIsFromPasteboard:NO requiresSpendingAuthenticationPrompt:NO keepAuthenticatedIfErrorAfterAuthentication:NO requestingAdditionalInfo:^(DSRequestingAdditionalInfo additionalInfoRequestType) {
            ...
            return TRUE; //just continue and let Axe Sync do it's thing
