...
-(NSPredicate*)searchPredicate {
    ...
    if (self.searchString && ![self.searchString isEqualToString:@""]) {
        if ([self.searchString isEqualToString:@"0"] || [self.searchString longLongValue]) {
            return [NSPredicate predicateWithFormat:@"type == %@ && governanceObjectHash.chain == %@ && (identifier == %@)",@(DSGovernanceObjectType_Proposal),[self.chainManager.chain chainEntityInContext:self.managedObjectContext],@([self.searchString longLongValue])];
        } else {
            return [NSPredicate predicateWithFormat:@"type == %@ && governanceObjectHash.chain == %@",@(DSGovernanceObjectType_Proposal),[self.chainManager.chain chainEntityInContext:self.managedObjectContext]];
        }
        //        else {
...
-(void)configureCell:(DSProposalTableViewCell*)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.amountLabel.attributedText = [[DSPriceManager sharedInstance] attributedStringForAxeAmount:governanceObjectEntity.amount];
