...
+ (instancetype)contactBasedDerivationPathWithDestinationBlockchainIdentityUniqueId:(UInt256) destinationBlockchainIdentityUniqueId sourceBlockchainIdentityUniqueId:(UInt256)sourceBlockchainIdentityUniqueId forAccountNumber:(uint32_t)accountNumber onChain:(DSChain*)chain {
    ...
    UInt256 indexes[] = {uint256_from_long(FEATURE_PURPOSE), uint256_from_long(coinType), uint256_from_long(FEATURE_PURPOSE_AXEPAY), uint256_from_long(accountNumber), sourceBlockchainIdentityUniqueId,destinationBlockchainIdentityUniqueId};
...
-(void)loadAddressesInContext:(NSManagedObjectContext*)context {
    if (!self.addressesLoaded) {
        [context performBlockAndWait:^{
            ...
            for (DSAddressEntity *e in derivationPathEntity.addresses) {
                @autoreleasepool {
                    ...
                    if (![e.address isValidAxeAddressOnChain:self.account.wallet.chain]) {
        ...
        [self registerAddressesWithGapLimit:SEQUENCE_AXEPAY_GAP_LIMIT_INITIAL inContext:context error:nil];
    ...
    @synchronized(self) {
        ...
        while (a.count < upperLimit) { // generate new addresses up to gapLimit
            ...
            if (! address) {
                ...
                if (error) {
                    *error = [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
            ...
            if (!self.account.wallet.isTransient) {
                [context performBlockAndWait:^{ // store new address in core data
                    ...
                    NSAssert([address isValidAxeAddressOnChain:self.chain], @"the address is being saved to the wrong derivation path");
                    e.address = address;
                    e.index = n;
                    e.internal = NO;
                    e.standalone = NO;
