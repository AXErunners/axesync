...
@property(nonatomic,strong) DPDocumentFactory* axepayDocumentFactory;
@property(nonatomic,strong) DPDocumentFactory* dpnsDocumentFactory;

@property(nonatomic,strong) DSAxepayUserEntity * matchingAxepayUserInViewContext;

@property(nonatomic,strong) DSAxepayUserEntity * matchingAxepayUserInPlatformContext;

@property (nonatomic, strong) DSChain * chain;

@property (nonatomic, strong) DSECDSAKey * registrationFundingPrivateKey;

@property (nonatomic, assign) UInt256 axepaySyncronizationBlockHash;
...
-(instancetype)initWithUniqueId:(UInt256)uniqueId isTransient:(BOOL)isTransient onChain:(DSChain*)chain {
    ...
    _identityQueue = dispatch_queue_create([[NSString stringWithFormat:@"org.axecore.axesync.identity.%@",uint256_base58(uniqueId)] UTF8String], DISPATCH_QUEUE_SERIAL);
    self.chain = chain;
    return self;
}
...
-(void)applyIdentityEntity:(DSBlockchainIdentityEntity*)blockchainIdentityEntity {
    ...
    self.axepaySyncronizationBlockHash = blockchainIdentityEntity.axepaySyncronizationBlockHash.UInt256;
...
//                }
    ...
    self.matchingAxepayUserInViewContext = [[NSManagedObjectContext viewContext] objectWithID:blockchainIdentityEntity.matchingAxepayUser.objectID];
    self.matchingAxepayUserInPlatformContext = [[NSManagedObjectContext platformContext] objectWithID:blockchainIdentityEntity.matchingAxepayUser.objectID];
}
...
-(instancetype)initAtIndex:(uint32_t)index withLockedOutpoint:(DSUTXO)lockedOutpoint inWallet:(DSWallet*)wallet {
    ...
    _identityQueue = dispatch_queue_create([[NSString stringWithFormat:@"org.axecore.axesync.identity.%@",uint256_base58(self.uniqueID)] UTF8String], DISPATCH_QUEUE_SERIAL);
    return self;
}
...
-(void)continueRegisteringOnNetwork:(DSBlockchainIdentityRegistrationStep)steps withFundingAccount:(DSAccount*)fundingAccount forTopupAmount:(uint64_t)topupHakAmount stepCompletion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationStep stepCompleted))stepCompletion completion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationStep stepsCompleted, NSError * error))completion {
    [self continueRegisteringOnNetwork:steps withFundingAccount:fundingAccount forTopupAmount:topupHakAmount inContext:self.platformContext stepCompletion:stepCompletion completion:completion];
}

-(void)continueRegisteringOnNetwork:(DSBlockchainIdentityRegistrationStep)steps withFundingAccount:(DSAccount*)fundingAccount forTopupAmount:(uint64_t)topupHakAmount inContext:(NSManagedObjectContext*)context stepCompletion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationStep stepCompleted))stepCompletion completion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationStep stepsCompleted, NSError * error))completion {
    if (!self.registrationCreditFundingTransaction) {
        [self registerOnNetwork:steps withFundingAccount:fundingAccount forTopupAmount:topupHakAmount stepCompletion:stepCompletion completion:completion];
    ...
    } else if ([self matchingAxepayUserInContext:context].remoteProfileDocumentRevision < 1) {
...
-(void)registerOnNetwork:(DSBlockchainIdentityRegistrationStep)steps withFundingAccount:(DSAccount*)fundingAccount forTopupAmount:(uint64_t)topupHakAmount stepCompletion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationStep stepCompleted))stepCompletion completion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationStep stepsCompleted, NSError * error))completion {
    ...
    if (![self hasBlockchainIdentityExtendedPublicKeys]) {
        if (completion) {
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(stepsCompleted, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey: DSLocalizedString(@"The blockchain identity extended public keys need to be registered before you can register a blockchain identity.", nil)}]);
            });
        }
        return;
    }
    ...
    [self fundingTransactionForTopupAmount:topupHakAmount toAddress:creditFundingRegistrationAddress fundedByAccount:fundingAccount completion:^(DSCreditFundingTransaction * _Nonnull fundingTransaction) {
        if (!fundingTransaction) {
            if (completion) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    completion(stepsCompleted, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey: DSLocalizedString(@"Funding transaction could not be created", nil)}]);
                });
            }
            return;
        }
            ...
            if (!signedTransaction) {
                if (completion) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        completion(stepsCompleted, cancelled?nil:[NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey: DSLocalizedString(@"Transaction could not be signed", nil)}]);
                    });
                }
                return;
            }
            ...
            [self.chain.chainManager.transactionManager publishTransaction:fundingTransaction completion:^(NSError * _Nullable error) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0),^{
                    ...
                    if (!transactionSuccessfullyPublished) {
                        if (completion) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                completion(stepsCompleted, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:DSLocalizedString(@"Timeout while waiting for funding transaction to be accepted by network", nil)}]);
                            });
                        }
                        return;
                    }
...
-(void)registerInWalletForBlockchainIdentityUniqueId:(UInt256)blockchainIdentityUniqueId {
    ...
    if (!self.identityQueue) {
        _identityQueue = dispatch_queue_create([[NSString stringWithFormat:@"org.axecore.axesync.identity.%@",uint256_base58(blockchainIdentityUniqueId)] UTF8String], DISPATCH_QUEUE_SERIAL);
    }
    [self registerInWallet];
}
...
-(DSAxepayUserEntity*)matchingAxepayUserInViewContext {
    if (!_matchingAxepayUserInViewContext) {
        _matchingAxepayUserInViewContext = [self matchingAxepayUserInContext:[NSManagedObjectContext viewContext]];
    }
    return _matchingAxepayUserInViewContext;
}

-(DSAxepayUserEntity*)matchingAxepayUserInPlatformContext {
    if (!_matchingAxepayUserInPlatformContext) {
        _matchingAxepayUserInPlatformContext = [self matchingAxepayUserInContext:[NSManagedObjectContext platformContext]];
    }
    return _matchingAxepayUserInPlatformContext;
}

-(DSAxepayUserEntity*)matchingAxepayUserInContext:(NSManagedObjectContext*)context {
    if (_matchingAxepayUserInViewContext || _matchingAxepayUserInPlatformContext) {
        if (context == [_matchingAxepayUserInPlatformContext managedObjectContext]) return _matchingAxepayUserInPlatformContext;
        if (context == [_matchingAxepayUserInViewContext managedObjectContext]) return _matchingAxepayUserInViewContext;
        if (_matchingAxepayUserInPlatformContext) {
            return [context objectWithID:_matchingAxepayUserInPlatformContext.objectID];
        } else {
            return [context objectWithID:_matchingAxepayUserInViewContext.objectID];
        }
    } else {
        __block DSAxepayUserEntity * axepayUserEntity = nil;
        [context performBlockAndWait:^{
            axepayUserEntity = [DSAxepayUserEntity anyObjectInContext:context matching:@"associatedBlockchainIdentity.uniqueID == %@",uint256_data(self.uniqueID)];
        }];
        return axepayUserEntity;
    }
}
...
-(NSString*)currentAxepayUsername {
    return [self.axepayUsernames firstObject];
}
...
-(NSString*)localizedRegistrationStatusString {
    switch (self.registrationStatus) {
        case DSBlockchainIdentityRegistrationStatus_Registered:
            return DSLocalizedString(@"Registered", @"The Axe Identity is registered");
            break;
        case DSBlockchainIdentityRegistrationStatus_Unknown:
            return DSLocalizedString(@"Unknown", @"It is Unknown if the Axe Identity is registered");
            break;
        case DSBlockchainIdentityRegistrationStatus_Registering:
            return DSLocalizedString(@"Registering", @"The Axe Identity is being registered");
            break;
        case DSBlockchainIdentityRegistrationStatus_NotRegistered:
            return DSLocalizedString(@"Not Registered", @"The Axe Identity is not registered");
            break;
...
-(void)registrationTransitionWithCompletion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationTransition * _Nullable blockchainIdentityRegistrationTransaction, NSError * _Nullable error))completion {
    if (!self.registrationFundingPrivateKey) {
        if (completion) {
            completion(nil,[NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:DSLocalizedString(@"The blockchain identity funding private key should be first created with createFundingPrivateKeyWithCompletion", nil)}]);
        }
        return;
    }
        ...
        if (blockchainIdentityRegistrationTransition) {
            [self.DAPIClient publishTransition:blockchainIdentityRegistrationTransition success:^(NSDictionary * _Nonnull successDictionary) {
                ...
                if (error) {
                    ...
                    if (completion) {
                        completion(nil,[NSError errorWithDomain:@"AxeSync" code:501 userInfo:@{NSLocalizedDescriptionKey:
            ...
            if (completion) {
                NSError * error = [NSError errorWithDomain:@"AxeSync" code:501 userInfo:@{NSLocalizedDescriptionKey:
...
-(void)fetchAllNetworkStateInformationInContext:(NSManagedObjectContext*)context withCompletion:(void (^)(DSBlockchainIdentityQueryStep failureStep, NSArray<NSError *> * errors))completion {
    dispatch_async(self.identityQueue, ^{
        ...
        if ([DSOptionsManager sharedInstance].syncType & DSSyncType_Axepay) {
...
-(void)fetchL3NetworkStateInformation:(DSBlockchainIdentityQueryStep)queryStep inContext:(NSManagedObjectContext*)context withCompletion:(void (^)(DSBlockchainIdentityQueryStep failureStep, NSArray<NSError *> * errors))completion {
    if (!(queryStep & DSBlockchainIdentityQueryStep_Identity) && (!self.activeKeyCount)) {
        ...
        if (completion) {
            completion(DSBlockchainIdentityQueryStep_BadQuery,@[[NSError errorWithDomain:@"AxeSync" code:501 userInfo:@{NSLocalizedDescriptionKey:DSLocalizedString(@"Attempt to query DAPs for blockchain identity with no active keys", nil)}]]);
        }
        return;
    }
...
//    }
    ...
    if (completion) {
        dispatch_group_notify(dispatchGroup, dispatch_get_main_queue(), ^{
            DSDLog(@"Completed fetching of blockchain identity information for user %@ (query %lu - failures %lu)",self.currentAxepayUsername?self.currentAxepayUsername:self.uniqueIdString,(unsigned long)queryStep,failureStep);
            if (!(failureStep & DSBlockchainIdentityQueryStep_ContactRequests)) {
                ...
                strongSelf.axepaySyncronizationBlockHash = strongSelf.chain.lastTerminalBlock.blockHash;
            }
    ...
    dispatch_async(self.identityQueue, ^{
        if (!self.activeKeyCount) {
            if (self.isLocal) {
                ...
                if (![self.axepayUsernameFullPaths count] && self.lastCheckedUsernamesTimestamp == 0 && [DSOptionsManager sharedInstance].syncType & DSSyncType_DPNS) {
                    stepsNeeded |= DSBlockchainIdentityQueryStep_Username;
                }
                if ((self.lastCheckedProfileTimestamp < [[NSDate date] timeIntervalSince1970] - HOUR_TIME_INTERVAL) && [DSOptionsManager sharedInstance].syncType & DSSyncType_Axepay) {
            ...
            if (![[self matchingAxepayUserInContext:context] createdAt] && (self.lastCheckedProfileTimestamp < [[NSDate date] timeIntervalSince1970] - HOUR_TIME_INTERVAL) && [DSOptionsManager sharedInstance].syncType & DSSyncType_Axepay) {
                stepsNeeded |= DSBlockchainIdentityQueryStep_Profile;
            }
            if (self.isLocal && (self.lastCheckedIncomingContactsTimestamp < [[NSDate date] timeIntervalSince1970] - HOUR_TIME_INTERVAL) && [DSOptionsManager sharedInstance].syncType & DSSyncType_Axepay) {
                stepsNeeded |= DSBlockchainIdentityQueryStep_IncomingContactRequests;
            }
            if (self.isLocal && (self.lastCheckedOutgoingContactsTimestamp < [[NSDate date] timeIntervalSince1970] - HOUR_TIME_INTERVAL) && [DSOptionsManager sharedInstance].syncType & DSSyncType_Axepay) {
...
-(DPDocumentFactory*)axepayDocumentFactory {
    if (!_axepayDocumentFactory) {
        DPContract * contract = [DSAxePlatform sharedInstanceForChain:self.chain].axePayContract;
        NSAssert(contract,@"Contract must be defined");
        self.axepayDocumentFactory = [[DPDocumentFactory alloc] initWithBlockchainIdentity:self contract:contract onChain:self.chain];
    }
    return _axepayDocumentFactory;
}

-(DPDocumentFactory*)dpnsDocumentFactory {
    if (!_dpnsDocumentFactory) {
        DPContract * contract = [DSAxePlatform sharedInstanceForChain:self.chain].dpnsContract;
        NSAssert(contract,@"Contract must be defined");
...
-(void)fetchAndUpdateContract:(DPContract*)contract inContext:(NSManagedObjectContext*)context {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{ //this is so we don't get DAPINetworkService immediately
        ...
        BOOL isAxepayEmpty = [contract.name isEqual:@"AxePay"] && uint256_is_zero(self.chain.axepayContractID);
        if (((isDPNSEmpty || isAxepayEmpty) && uint256_is_zero(contract.registeredBlockchainIdentityUniqueID)) || contract.contractState == DPContractState_NotRegistered) {
...
-(void)addAxepayUsername:(NSString*)username save:(BOOL)save {
    [self addUsername:username inDomain:[self axepayDomainName] status:DSBlockchainIdentityUsernameStatus_Initial save:save registerOnNetwork:YES];
}
...
-(DSBlockchainIdentityUsernameStatus)statusOfAxepayUsername:(NSString*)username {
    return [self statusOfUsernameFullPath:[self fullPathForUsername:username inDomain:[self axepayDomainName]]];
}
...
-(NSArray<NSString*>*)axepayUsernameFullPaths {
    return [self.usernameStatuses allKeys];
}

-(NSArray<NSString*>*)axepayUsernames {
...
-(NSArray<DPDocument*>*)domainDocumentsForUnregisteredUsernameFullPaths:(NSArray*)unregisteredUsernameFullPaths usingEntropyString:(NSString*)entropyString inContext:(NSManagedObjectContext*)context error:(NSError**)error {
    ...
    for (NSString * usernameFullPath in [self saltedDomainHashesForUsernameFullPaths:unregisteredUsernameFullPaths inContext:context]) {
        ...
        DSStringValueDictionary * dataDictionary = @{
            ...
            @"records" : @{@"axeUniqueIdentityId":uint256_base58(self.uniqueID)},
...
-(void)registerPreorderedSaltedDomainHashesForUsernameFullPaths:(NSArray*)usernameFullPaths inContext:(NSManagedObjectContext*)context completion:(void (^ _Nullable)(BOOL success, NSError * error))completion {
    ...
    [self signStateTransition:transition completion:^(BOOL success) {
        if (success) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (completion) {
                    completion(NO,[NSError errorWithDomain:@"AxeSync" code:501 userInfo:@{NSLocalizedDescriptionKey:
...
- (void)fetchUsernamesInContext:(NSManagedObjectContext*)context withCompletion:(void (^)(BOOL success, NSError * error))completion {
    ...
    if (contract.contractState != DPContractState_Registered) {
        if (completion) {
            completion(NO, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
    ...
    [self.DAPINetworkService getDPNSDocumentsForUsernames:usernames inDomain:domain success:^(id _Nonnull domainDocumentArray) {
        ...
        if ([domainDocumentArray isKindOfClass:[NSArray class]]) {
            ...
            for (NSString * username in usernames) {
                for (NSDictionary * domainDocument in domainDocumentArray) {
                    ...
                    if ([normalizedLabel isEqualToString:[username lowercaseString]]) {
                        ...
                        [usernameStatusDictionary setObject:@(DSBlockchainIdentityUsernameStatus_Confirmed) forKey:BLOCKCHAIN_USERNAME_STATUS];
                        [self.usernameStatuses setObject:[usernameStatusDictionary copy] forKey:[self fullPathForUsername:username inDomain:[self axepayDomainName]]];
            ...
            if ([usernamesLeft count] && retryCount > 0) {
                ...
                if (completion) {
                    completion(FALSE, [NSError errorWithDomain:@"AxeSync" code:501 userInfo:@{NSLocalizedDescriptionKey:
...
-(DPDocument*)matchingAxepayUserProfileDocumentInContext:(NSManagedObjectContext*)context {
    //The revision must be at least at 1, otherwise nothing was ever done
    DSAxepayUserEntity * matchingAxepayUser = [self matchingAxepayUserInContext:context];
    if (matchingAxepayUser && matchingAxepayUser.localProfileDocumentRevision) {
        ...
        [context performBlockAndWait:^{
            ...
            dataDictionary = [@{
                @"$updatedAt": @(matchingAxepayUser.updatedAt),
            } mutableCopy];
            if (matchingAxepayUser.createdAt == matchingAxepayUser.updatedAt) {
                dataDictionary[@"$createdAt"] = @(matchingAxepayUser.createdAt);
            } else {
                dataDictionary[@"$revision"] = @(matchingAxepayUser.localProfileDocumentRevision);
            }
            if (matchingAxepayUser.publicMessage) {
                dataDictionary[@"publicMessage"] = matchingAxepayUser.publicMessage;
            }
            if (matchingAxepayUser.avatarPath) {
                dataDictionary[@"avatarUrl"] = matchingAxepayUser.avatarPath;
            }
            if (matchingAxepayUser.displayName) {
                dataDictionary[@"displayName"] = matchingAxepayUser.displayName;
            }
            entropyString = matchingAxepayUser.originalEntropyString;
            documentIdentifier = matchingAxepayUser.documentIdentifier;
        }];
        NSError * error = nil;
        if (documentIdentifier == nil) {
            NSAssert(entropyString, @"Entropy string must be present");
            DPDocument * document = [self.axepayDocumentFactory documentOnTable:@"profile" withDataDictionary:dataDictionary usingEntropy:entropyString error:&error];
            return document;
        } else {
            DPDocument * document = [self.axepayDocumentFactory documentOnTable:@"profile" withDataDictionary:dataDictionary usingDocumentIdentifier:documentIdentifier error:&error];
            return document;
        }
    } else {
        return nil;
...
- (DSBlockchainIdentityFriendshipStatus)friendshipStatusForRelationshipWithBlockchainIdentity:(DSBlockchainIdentity*)otherBlockchainIdentity {
    if (!self.matchingAxepayUserInViewContext) return DSBlockchainIdentityFriendshipStatus_Unknown;
    __block BOOL isIncoming;
    __block BOOL isOutgoing;
    [self.matchingAxepayUserInViewContext.managedObjectContext performBlockAndWait:^{
        isIncoming = !![self.matchingAxepayUserInViewContext.incomingRequests filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"sourceContact.associatedBlockchainIdentity.uniqueID == %@", uint256_data(otherBlockchainIdentity.uniqueID)]].count;
        isOutgoing = !![self.matchingAxepayUserInViewContext.outgoingRequests filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"destinationContact.associatedBlockchainIdentity.uniqueID == %@", uint256_data(otherBlockchainIdentity.uniqueID)]].count;
    }];
...
-(void)setAxepaySyncronizationBlockHash:(UInt256)axepaySyncronizationBlockHash {
    _axepaySyncronizationBlockHash = axepaySyncronizationBlockHash;
    if (uint256_is_zero(_axepaySyncronizationBlockHash)) {
        _axepaySyncronizationBlockHeight = 0;
    } else {
        _axepaySyncronizationBlockHeight = [self.chain heightForBlockHash:_axepaySyncronizationBlockHash];
        if (_axepaySyncronizationBlockHeight == UINT32_MAX) {
            _axepaySyncronizationBlockHeight = 0;
        }
    }
}
...
- (void)sendNewFriendRequestToBlockchainIdentity:(DSBlockchainIdentity*)blockchainIdentity inContext:(NSManagedObjectContext*)context completion:(void (^)(BOOL success, NSArray<NSError *> * _Nullable errors))completion {
    ...
    [blockchainIdentity fetchNeededNetworkStateInformationWithCompletion:^(DSBlockchainIdentityQueryStep failureStep, NSArray<NSError *> * _Nullable errors) {
        ...
        if (![blockchainIdentity isAxepayReady]) {
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(NO, @[[NSError errorWithDomain:@"AxeSync" code:501 userInfo:@{NSLocalizedDescriptionKey:
                                                                                            DSLocalizedString(@"User has actions to complete before being able to use Axepay", nil)}]]);
            });
...
- (void)sendNewFriendRequestToPotentialContact:(DSPotentialContact*)potentialContact completion:(void (^)(BOOL success, NSArray<NSError *> * _Nullable errors))completion {
    ...
    if (!_isLocal) return;
    __weak typeof(self) weakSelf = self;
    [self.DAPINetworkService getIdentityByName:potentialContact.username inDomain:[self axepayDomainName] success:^(NSDictionary *_Nonnull blockchainIdentityDictionary) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            if (completion) {
                completion(NO, @[[NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
...
- (void)sendNewFriendRequestMatchingPotentialFriendship:(DSPotentialOneWayFriendship*)potentialFriendship inContext:(NSManagedObjectContext*)context completion:(void (^)(BOOL success, NSArray<NSError *> * errors))completion {
    NSAssert(_isLocal, @"This should not be performed on a non local blockchain identity");
    if (!_isLocal) return;
    DSAxepayUserEntity * destinationAxepayUser = [potentialFriendship.destinationBlockchainIdentity matchingAxepayUserInContext:context];
    if (!destinationAxepayUser) {
        NSAssert([potentialFriendship.destinationBlockchainIdentity matchingAxepayUserInContext:context], @"There must be a destination contact if the destination blockchain identity is not known");
        return;
    }
...
//             DSFriendRequestEntity * friendRequest = [potentialFriendship outgoingFriendRequestForAxepayUserEntity:potentialFriendship.destinationBlockchainIdentity.matchingAxepayUser];
//                   [strongSelf.matchingAxepayUser addOutgoingRequestsObject:friendRequest];
//
//                   if ([[friendRequest.destinationContact.outgoingRequests filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"destinationContact == %@",strongSelf.matchingAxepayUser]] count]) {
//                       [strongSelf.matchingAxepayUser addFriendsObject:friendRequest.destinationContact];
//                   }
...
-(void)acceptFriendRequestFromBlockchainIdentity:(DSBlockchainIdentity*)otherBlockchainIdentity inContext:(NSManagedObjectContext*)context completion:(void (^)(BOOL success, NSArray<NSError *> * errors))completion {
    ...
    [context performBlockAndWait:^{
        DSAxepayUserEntity * matchingAxepayUser = [self matchingAxepayUserInContext:context];
        DSFriendRequestEntity * friendRequest = [[matchingAxepayUser.incomingRequests filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"sourceContact.associatedBlockchainIdentity.uniqueID == %@", uint256_data(otherBlockchainIdentity.uniqueID)]] anyObject];
        if (!friendRequest) {
            if (completion) {
                completion(NO,@[[NSError errorWithDomain:@"AxeSync" code:501 userInfo:@{NSLocalizedDescriptionKey:
...
-(void)acceptFriendRequest:(DSFriendRequestEntity*)friendRequest completion:(void (^)(BOOL success, NSArray<NSError *> * errors))completion {
    ...
    DSAccount * account = [self.wallet accountWithNumber:0];
    DSAxepayUserEntity * otherAxepayUser = friendRequest.sourceContact;
    DSBlockchainIdentity * otherBlockchainIdentity = [self.chain blockchainIdentityForUniqueId:otherAxepayUser.associatedBlockchainIdentity.uniqueID.UInt256];
    
    if (!otherBlockchainIdentity) {
        otherBlockchainIdentity = [[DSBlockchainIdentity alloc] initWithBlockchainIdentityEntity:otherAxepayUser.associatedBlockchainIdentity];
    }
...
-(DSDocumentTransition*)profileDocumentTransitionInContext:(NSManagedObjectContext*)context {
    DPDocument * profileDocument = [self matchingAxepayUserProfileDocumentInContext:context];
...
- (void)updateAxepayProfileWithDisplayName:(NSString*)displayName publicMessage:(NSString*)publicMessage avatarURLString:(NSString *)avatarURLString {
    [self updateAxepayProfileWithDisplayName:displayName publicMessage:publicMessage avatarURLString:avatarURLString inContext:self.platformContext];
}

- (void)updateAxepayProfileWithDisplayName:(NSString*)displayName publicMessage:(NSString*)publicMessage avatarURLString:(NSString *)avatarURLString inContext:(NSManagedObjectContext*)context {
    [context performBlockAndWait:^{
        DSAxepayUserEntity * matchingAxepayUser = [self matchingAxepayUserInContext:context];
        matchingAxepayUser.displayName = displayName;
        matchingAxepayUser.publicMessage = publicMessage;
        matchingAxepayUser.avatarPath = avatarURLString;
        if (!matchingAxepayUser.remoteProfileDocumentRevision) {
            matchingAxepayUser.createdAt = [[NSDate date] timeIntervalSince1970] * 1000;
            if (!matchingAxepayUser.originalEntropyString) {
                matchingAxepayUser.originalEntropyString = [DSKey randomAddressForChain:self.chain];
            }
        }
        matchingAxepayUser.updatedAt = [[NSDate date] timeIntervalSince1970] * 1000;
        matchingAxepayUser.localProfileDocumentRevision++;
        [context ds_save];
    }];
}
...
-(void)signedProfileDocumentTransitionInContext:(NSManagedObjectContext*)context withCompletion:(void (^)(DSTransition * transition, BOOL cancelled, NSError * error))completion {
    ...
    if (!transition) {
        if (completion) {
            completion(nil, NO, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
...
- (void)signAndPublishProfileInContext:(NSManagedObjectContext*)context withCompletion:(void (^)(BOOL success, BOOL cancelled, NSError * error))completion {
    ...
    [context performBlockAndWait:^{
        profileDocumentRevision = [self matchingAxepayUserInContext:context].localProfileDocumentRevision;
        [context ds_save];
    }];
    [self signedProfileDocumentTransitionInContext:context withCompletion:^(DSTransition *transition, BOOL cancelled, NSError *error) {
        ...
        [self.DAPINetworkService publishTransition:transition success:^(NSDictionary * _Nonnull successDictionary) {
            ...
            if (!strongSelf) {
                if (completion) {
                    completion(NO, NO, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
                                                                                                    DSLocalizedString(@"Internal memory allocation error", nil)}]);
                }
                return;
            }
            [context performBlockAndWait:^{
                [self matchingAxepayUserInContext:context].remoteProfileDocumentRevision = profileDocumentRevision;
                [context ds_save];
            }];
            if (completion) {
                dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)fetchProfileInContext:(NSManagedObjectContext*)context withCompletion:(void (^)(BOOL success, NSError * error))completion {
    ...
    DPContract * axepayContract = [DSAxePlatform sharedInstanceForChain:self.chain].axePayContract;
    if ([axepayContract contractState] != DPContractState_Registered) {
        if (completion) {
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(NO, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
                                                                                            DSLocalizedString(@"Axepay Contract is not yet registered on network", nil)}]);
            });
        }
        return;
    }
...
- (void)fetchProfileForBlockchainIdentityUniqueId:(UInt256)blockchainIdentityUniqueId saveReturnedProfile:(BOOL)saveReturnedProfile inContext:(NSManagedObjectContext*)context completion:(void (^)(BOOL success, NSError * error))completion {
    ...
    if ([axepayContract contractState] != DPContractState_Registered) {
        if (completion) {
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(NO, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
                                                                                            DSLocalizedString(@"The Axepay contract is not properly set up", nil)}]);
            });
        }
        return;
    }
    
    [self.DAPINetworkService getAxepayProfileForUserId:uint256_base58(blockchainIdentityUniqueId) success:^(NSArray<NSDictionary *> * _Nonnull documents) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            if (completion) {
                completion(NO, [NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
        ...
        dispatch_async(self.identityQueue, ^{
            ...
            [context performBlockAndWait:^{
                ...
                DSAxepayUserEntity * contact = [[self blockchainIdentityEntityInContext:context] matchingAxepayUser];
                if (!contact) {
                    NSAssert(FALSE, @"It is weird to get here");
                    contact = [DSAxepayUserEntity anyObjectInContext:context matching:@"associatedBlockchainIdentity.uniqueID == %@", uint256_data(blockchainIdentityUniqueId)];
                }
                if (!contact || [[contactDictionary objectForKey:@"$updatedAt"] unsignedLongValue] > contact.updatedAt) {
                    
                    if (!contact) {
                        contact = [DSAxepayUserEntity managedObjectInBlockedContext:context];
                        contact.chain = [strongSelf.wallet.chain chainEntityInContext:context];
                        DSBlockchainIdentity * blockchainIdentity;
                        if (uint256_eq(blockchainIdentityUniqueId, strongSelf.uniqueID) && (![strongSelf matchingAxepayUserInContext:context])) {
                        ...
                        } else if ((blockchainIdentity = [strongSelf.wallet blockchainIdentityForUniqueId:blockchainIdentityUniqueId]) && ![blockchainIdentity matchingAxepayUserInContext:context]) {
...
- (void)fetchIncomingContactRequestsInContext:(NSManagedObjectContext*)context withCompletion:(void (^)(BOOL success, NSArray<NSError *> *errors))completion {
    ...
    if (![self activePrivateKeysAreLoadedWithFetchingError:&error]) {
        ...
        if (completion) {
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(NO, @[error?error:[NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
    ...
    __weak typeof(self) weakSelf = self;
    [self.DAPINetworkService getAxepayIncomingContactRequestsForUserId:self.uniqueIDData since:self.lastCheckedIncomingContactsTimestamp?(self.lastCheckedIncomingContactsTimestamp - HOUR_TIME_INTERVAL):0 success:^(NSArray<NSDictionary *> * _Nonnull documents) {
...
- (void)fetchOutgoingContactRequestsInContext:(NSManagedObjectContext*)context withCompletion:(void (^)(BOOL success,  NSArray<NSError *> *errors))completion {
    ...
    __weak typeof(self) weakSelf = self;
    [self.DAPINetworkService getAxepayOutgoingContactRequestsForUserId:self.uniqueIdString since:self.lastCheckedOutgoingContactsTimestamp?(self.lastCheckedOutgoingContactsTimestamp - HOUR_TIME_INTERVAL):0 success:^(NSArray<NSDictionary *> * _Nonnull documents) {
        //todo chance the since parameter
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            if (completion) {
...
- (void)handleContactRequestObjects:(NSArray<NSDictionary *> *)rawContactRequests context:(NSManagedObjectContext *)context completion:(void (^)(BOOL success, NSArray<NSError *> * errors))completion {
    ...
    for (NSDictionary *rawContact in rawContactRequests) {
        ...
        if (uint256_eq(contactRequest.recipientBlockchainIdentityUniqueId, self.uniqueID)) {
            ...
            DSFriendRequestEntity * friendRequest = [DSFriendRequestEntity anyObjectInContext:context matching:@"destinationContact == %@ && sourceContact.associatedBlockchainIdentity.uniqueID == %@",[self matchingAxepayUserInContext:context],uint256_data(contactRequest.senderBlockchainIdentityUniqueId)];
            if (!friendRequest) {
            ...
            BOOL isNew = ![DSFriendRequestEntity countObjectsInContext:context matching:@"sourceContact == %@ && destinationContact.associatedBlockchainIdentity.uniqueID == %@",[self matchingAxepayUserInContext:context],[NSData dataWithUInt256:contactRequest.recipientBlockchainIdentityUniqueId]];
            if (isNew) {
    ...
    [context performBlockAndWait:^{
        ...
        for (DSContactRequest * contactRequest in incomingRequests) {
            ...
            if (!externalBlockchainIdentityEntity) {
                //no externalBlockchainIdentity exists yet, which means no axepay user
                dispatch_group_enter(dispatchGroup);
                ...
                [senderBlockchainIdentity fetchNeededNetworkStateInformationWithCompletion:^(DSBlockchainIdentityQueryStep failureStep, NSArray<NSError *> * _Nullable networkErrors) {
                    if (!failureStep) {
                        ...
                        if (!extendedPublicKey) {
                            succeeded = FALSE;
                            [errors addObject:[NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:
                                                                                                           DSLocalizedString(@"Incorrect key format after contact request decryption", nil)}]];
                        } else {
                            DSAxepayUserEntity * senderAxepayUserEntity = [senderBlockchainIdentity blockchainIdentityEntityInContext:context].matchingAxepayUser;
                            NSAssert(senderAxepayUserEntity, @"The sender should exist");
                            [self addIncomingRequestFromContact:senderAxepayUserEntity
                ...
                if ([self.chain blockchainIdentityForUniqueId:externalBlockchainIdentityEntity.uniqueID.UInt256]) {
                    ...
                    [potentialFriendship createDerivationPathWithCompletion:^(BOOL success, DSIncomingFundsDerivationPath * _Nonnull incomingFundsDerivationPath) {
                        if (success) {
                            DSAxepayUserEntity * matchingAxepayUserInContext = [self matchingAxepayUserInContext:context];
                            DSFriendRequestEntity * friendRequest = [potentialFriendship outgoingFriendRequestForAxepayUserEntity:matchingAxepayUserInContext atTimestamp:contactRequest.createdAt];
                            [potentialFriendship storeExtendedPublicKeyAssociatedWithFriendRequest:friendRequest];
                            [matchingAxepayUserInContext addIncomingRequestsObject:friendRequest];
                            
                            if ([[friendRequest.sourceContact.incomingRequests filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"sourceContact == %@",matchingAxepayUserInContext]] count]) {
                                [matchingAxepayUserInContext addFriendsObject:friendRequest.sourceContact];
                            }
                    ...
                    if ([sourceBlockchainIdentity activeKeyCount] > 0 && [sourceBlockchainIdentity keyAtIndex:contactRequest.senderKeyIndex]) {
                        ...
                        if (!extendedPublicKey) {
                            succeeded = FALSE;
                            [errors addObject:[NSError errorWithDomain:@"AxeSync" code:500 userInfo:@{NSLocalizedDescriptionKey:DSLocalizedString(@"Contact request extended public key is incorrectly encrypted.", nil)}]];
                            return;
                        }
                        [self addIncomingRequestFromContact:externalBlockchainIdentityEntity.matchingAxepayUser
                                       forExtendedPublicKey:extendedPublicKey
                                                atTimestamp:contactRequest.createdAt];
                        
                        DSAxepayUserEntity * matchingAxepayUserInContext = [self matchingAxepayUserInContext:context];
                        if ([[externalBlockchainIdentityEntity.matchingAxepayUser.incomingRequests filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"sourceContact == %@",matchingAxepayUserInContext]] count]) {
                            [matchingAxepayUserInContext addFriendsObject:[externalBlockchainIdentityEntity matchingAxepayUser]];
                            [context ds_save];
                        }
...
-(void)addFriendship:(DSPotentialOneWayFriendship*)friendship inContext:(NSManagedObjectContext*)context completion:(void (^)(BOOL success, NSError * error))completion  {
    ...
    //DSFriendRequestEntity * friendRequestEntity = [friendship outgoingFriendRequestForAxepayUserEntity:friendship.destinationBlockchainIdentity.matchingAxepayUser];
    DSFriendRequestEntity * friendRequestEntity = [DSFriendRequestEntity managedObjectInBlockedContext:context];
    friendRequestEntity.sourceContact = [friendship.sourceBlockchainIdentity matchingAxepayUserInContext:context];
    friendRequestEntity.destinationContact = [friendship.destinationBlockchainIdentity matchingAxepayUserInContext:context];
    ...
    [friendship createDerivationPathWithCompletion:^(BOOL success, DSIncomingFundsDerivationPath * _Nonnull incomingFundsDerivationPath) {
        ...
        DSAxepayUserEntity * axepayUserInChildContext = [self matchingAxepayUserInContext:context];
        
        [axepayUserInChildContext addOutgoingRequestsObject:friendRequestEntity];
        
        if ([[[friendship.destinationBlockchainIdentity matchingAxepayUserInContext:context].outgoingRequests filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"destinationContact == %@",axepayUserInChildContext]] count]) {
            [axepayUserInChildContext addFriendsObject:[friendship.destinationBlockchainIdentity matchingAxepayUserInContext:context]];
        }
...
-(void)addIncomingRequestFromContact:(DSAxepayUserEntity*)axepayUserEntity
                forExtendedPublicKey:(DSKey*)extendedPublicKey
                         atTimestamp:(NSTimeInterval)timestamp {
    NSManagedObjectContext * context = axepayUserEntity.managedObjectContext;
    DSFriendRequestEntity * friendRequestEntity = [DSFriendRequestEntity managedObjectInBlockedContext:context];
    friendRequestEntity.sourceContact = axepayUserEntity;
    friendRequestEntity.destinationContact = [self matchingAxepayUserInContext:axepayUserEntity.managedObjectContext];
    ...
    DSAccountEntity * accountEntity = [DSAccountEntity accountEntityForWalletUniqueID:self.wallet.uniqueIDString index:account.accountNumber onChain:self.chain inContext:axepayUserEntity.managedObjectContext];
    ...
    DSIncomingFundsDerivationPath * derivationPath = [DSIncomingFundsDerivationPath externalDerivationPathWithExtendedPublicKey:extendedPublicKey withDestinationBlockchainIdentityUniqueId:[self matchingAxepayUserInContext:axepayUserEntity.managedObjectContext].associatedBlockchainIdentity.uniqueID.UInt256 sourceBlockchainIdentityUniqueId:axepayUserEntity.associatedBlockchainIdentity.uniqueID.UInt256 onChain:self.chain];
    ...
    [account addOutgoingDerivationPath:derivationPath forFriendshipIdentifier:friendRequestEntity.friendshipIdentifier inContext:axepayUserEntity.managedObjectContext];
    
    DSAxepayUserEntity * matchingAxepayUser = [self matchingAxepayUserInContext:axepayUserEntity.managedObjectContext];
    [matchingAxepayUser addIncomingRequestsObject:friendRequestEntity];
    
    if ([[friendRequestEntity.sourceContact.incomingRequests filteredSetUsingPredicate:[NSPredicate predicateWithFormat:@"sourceContact == %@",matchingAxepayUser]] count]) {
        [matchingAxepayUser addFriendsObject:friendRequestEntity.sourceContact];
    }
...
-(void)saveInitialInContext:(NSManagedObjectContext*)context {
    ...
    [context performBlockAndWait:^{
        ...
        for (NSString * usernameFullPath in self.usernameStatuses) {
            ...
            [entity setAxepayUsername:usernameEntity];
        }
        DSAxepayUserEntity * axepayUserEntity = [DSAxepayUserEntity managedObjectInBlockedContext:context];
        axepayUserEntity.chain = [self.chain chainEntityInContext:context];
        entity.matchingAxepayUser = axepayUserEntity;
        
        [context ds_saveInBlockAndWait];
        [[NSManagedObjectContext viewContext] performBlockAndWait:^{
            self.matchingAxepayUserInViewContext = [[NSManagedObjectContext viewContext] objectWithID:axepayUserEntity.objectID];
            self.matchingAxepayUserInPlatformContext = [[NSManagedObjectContext platformContext] objectWithID:axepayUserEntity.objectID];
        }];
        if ([self isLocal]) {
            dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)saveInContext:(NSManagedObjectContext*)context {
    ...
    [context performBlockAndWait:^{
        ...
        if (!uint256_eq(entity.axepaySyncronizationBlockHash.UInt256,self.axepaySyncronizationBlockHash)) {
            entity.axepaySyncronizationBlockHash = uint256_data(self.axepaySyncronizationBlockHash);
            changeOccured = YES;
            [updateEvents addObject:DSBlockchainIdentityUpdateEventAxepaySyncronizationBlockHash];
        }
    ...
    [context performBlockAndWait:^{
        ...
        if (!count) {
            ...
            if (key.privateKeyData) {
                ...
                DSDLog(@"Saving key at %@ for user %@",[self identifierForKeyAtPath:path fromDerivationPath:derivationPath],self.currentAxepayUsername);
            } else {
                ...
                DSDLog(@"Saving key after rederivation %@ for user %@",[self identifierForKeyAtPath:path fromDerivationPath:derivationPath],self.currentAxepayUsername);
            }
...
-(void)deletePersistentObjectAndSave:(BOOL)save inContext:(NSManagedObjectContext*)context {
    [context performBlockAndWait:^{
        ...
        if (blockchainIdentityEntity) {
            NSSet <DSFriendRequestEntity *>* friendRequests = [blockchainIdentityEntity.matchingAxepayUser outgoingRequests];
...
-(NSString*)debugDescription {
    return [[super debugDescription] stringByAppendingString:[NSString stringWithFormat:@" {%@-%@}",self.currentAxepayUsername,self.uniqueIdString]];
}
