...
@class DSWallet,DSBlockchainIdentityRegistrationTransition,DSBlockchainIdentityTopupTransition,DSBlockchainIdentityUpdateTransition,DSBlockchainIdentityCloseTransition,DSAccount,DSChain,DSTransition,DSAxepayUserEntity,DSPotentialOneWayFriendship,DSTransaction,DSFriendRequestEntity,DSPotentialContact,DSCreditFundingTransaction,DSDocumentTransition,DSKey,DPDocumentFactory;
...
typedef NS_ENUM(NSUInteger, DSBlockchainIdentityRegistrationStep) {
    ...
    DSBlockchainIdentityRegistrationStep_RegistrationStepsWithUsernameAndAxepayProfile = DSBlockchainIdentityRegistrationStep_RegistrationStepsWithUsername | DSBlockchainIdentityRegistrationStep_Profile,
    DSBlockchainIdentityRegistrationStep_All = DSBlockchainIdentityRegistrationStep_RegistrationStepsWithUsernameAndAxepayProfile,
...
FOUNDATION_EXPORT NSString* const DSBlockchainIdentityUpdateEventAxepaySyncronizationBlockHash;
...
/*! @brief This is if the blockchain identity is present in wallets or not. If this is false then the blockchain identity is known for example from being a axepay friend. */
...
/*! @brief Related to DPNS. This is the list of usernames that are associated to the identity in the domain "axe". These usernames however might not yet be registered or might be invalid. This can be used in tandem with the statusOfUsername: method */
@property (nonatomic,readonly) NSArray <NSString *> * axepayUsernames;

/*! @brief Related to DPNS. This is the list of usernames with their .axe domain that are associated to the identity in the domain "axe". These usernames however might not yet be registered or might be invalid. This can be used in tandem with the statusOfUsername: method */
@property (nonatomic,readonly) NSArray <NSString *> * axepayUsernameFullPaths;
...
@property (nullable,nonatomic,readonly) NSString * currentAxepayUsername;

/*! @brief Related to registering the identity. This is the address used to fund the registration of the identity. Axe sent to this address in the special credit funding transaction will be converted to L2 credits */
@property (nonatomic,readonly) NSString * registrationFundingAddress;

/*! @brief Related to Axepay. This is the users status message */
@property (nonatomic,readonly) NSString * axepayBioString;
...
/*! @brief In our system a contact is a vue on a blockchain identity for Axepay. A blockchain identity is therefore represented by a contact that will have relationships in the system. This is in the default backgroundContext. */
@property (nonatomic,readonly) DSAxepayUserEntity* matchingAxepayUserInViewContext;
...
/*! @brief This is a convenience factory to quickly make axepay documents */
@property (nonatomic,readonly) DPDocumentFactory* axepayDocumentFactory;
...
/*! @brief AxepaySyncronizationBlock represents the last L1 block height for which Axepay would be synchronized, if this isn't at the end of the chain then we need to query L2 to make sure we don't need to update our bloom filter */
@property (nonatomic,readonly) uint32_t axepaySyncronizationBlockHeight;

/*! @brief AxepaySyncronizationBlock represents the last L1 block hash for which Axepay would be synchronized */
@property (nonatomic,readonly) UInt256 axepaySyncronizationBlockHash;
...
-(void)registerOnNetwork:(DSBlockchainIdentityRegistrationStep)steps withFundingAccount:(DSAccount*)account forTopupAmount:(uint64_t)topupHakAmount stepCompletion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationStep stepCompleted))stepCompletion completion:(void (^ _Nullable)(DSBlockchainIdentityRegistrationStep stepsCompleted, NSError * error))completion;
...
// MARK: - Axepay Friendship Helpers
