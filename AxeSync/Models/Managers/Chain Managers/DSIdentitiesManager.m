...
-(void)registerForeignBlockchainIdentity:(DSBlockchainIdentity*)blockchainIdentity {
    NSAssert(!blockchainIdentity.isTransient, @"Axe Identity should no longer be transient");
...
- (NSArray*)unsyncedBlockchainIdentities {
    ...
    for (DSBlockchainIdentity * blockchainIdentity in [self.chain localBlockchainIdentities]) {
        ...
        } else if (self.chain.lastSyncBlockHeight > blockchainIdentity.axepaySyncronizationBlockHeight) {
            //If they are equal then the blockchain identity is synced
            //This is because the axepaySyncronizationBlock represents the last block for the bloom filter used in L1 should be considered valid
...
-(void)retrieveAllBlockchainIdentitiesChainStatesForWallet:(DSWallet*)wallet {
    for (DSBlockchainIdentity * identity in [wallet.blockchainIdentities allValues]) {
        if (identity.registrationStatus == DSBlockchainIdentityRegistrationStatus_Unknown) {
            ...
            if (!identity.currentAxepayUsername) {
                if (([[DSOptionsManager sharedInstance] syncType] & DSSyncType_DPNS)) {
                    [identity fetchUsernamesWithCompletion:^(BOOL success, NSError * error) {
...
- (id<DSDAPINetworkServiceRequest>)searchIdentitiesByAxepayUsernamePrefix:(NSString*)namePrefix withCompletion:(IdentitiesCompletionBlock)completion {
    return [self searchIdentitiesByNamePrefix:namePrefix inDomain:@"axe" offset:0 limit:100 withCompletion:completion];
}
...
- (id<DSDAPINetworkServiceRequest>)searchIdentityByAxepayUsername:(NSString*)name withCompletion:(IdentityCompletionBlock)completion {
    return [self searchIdentityByName:name inDomain:@"axe" withCompletion:completion];
}
...
- (id<DSDAPINetworkServiceRequest>)searchIdentitiesByAxepayUsernamePrefix:(NSString*)namePrefix offset:(uint32_t)offset limit:(uint32_t)limit withCompletion:(IdentitiesCompletionBlock)completion {
    return [self searchIdentitiesByNamePrefix:namePrefix inDomain:@"axe" offset:offset limit:limit withCompletion:completion];
}
    ...
    id<DSDAPINetworkServiceRequest> call = [client.DAPINetworkService searchDPNSDocumentsForUsernamePrefix:namePrefix inDomain:domain offset:offset limit:limit success:^(NSArray<NSDictionary *> * _Nonnull documents) {
        ...
        for (NSDictionary * document in documents) {
            ...
            if (!identity) {
                ...
                if (![identity.axepayUsernames containsObject:label]) {
