...
static NSString *const DEFAULT_SCHEMA = @"https://schema.axe.org/dpp-0-4-0/meta/data-contract";
...
-(NSString*)name {
    return [DSAxePlatform nameForContractWithIdentifier:self.localContractIdentifier];
}
...
+ (DPContract *)localAxepayContractForChain:(DSChain*)chain {
    // TODO: read async'ly
    NSString *bundlePath = [[NSBundle bundleForClass:self.class] pathForResource:@"AxeSync" ofType:@"bundle"];
    NSBundle *bundle = [NSBundle bundleWithPath:bundlePath];
    NSString *path = [bundle pathForResource:@"axepay-contract" ofType:@"json"];
    NSError *error = nil;
    ...
    NSString * localIdentifier = [NSString stringWithFormat:@"%@-%@",AXEPAY_CONTRACT,chain.uniqueID];
    ...
    if (!uint256_is_zero(chain.axepayContractID) && contract.contractState == DPContractState_Unknown) {
        [contract setContractState:DPContractState_Registered inContext:[NSManagedObjectContext platformContext]];
        contract.contractId = chain.axepayContractID;
