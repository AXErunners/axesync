...
+(NSURL*)storeWALURL {
    ...
    dispatch_once(&onceToken, ^{
        NSURL *docURL = [[NSFileManager defaultManager] URLsForDirectory:NSApplicationSupportDirectory inDomains:NSUserDomainMask].lastObject;
        NSString *fileName = @"AxeSync.sqlite-wal";
        storeURL = [docURL URLByAppendingPathComponent:fileName];
    });
    return storeURL;
}
...
+(NSURL*)storeSHMURL {
    ...
    dispatch_once(&onceToken, ^{
        NSURL *docURL = [[NSFileManager defaultManager] URLsForDirectory:NSApplicationSupportDirectory inDomains:NSUserDomainMask].lastObject;
        NSString *fileName = @"AxeSync.sqlite-shm";
        storeURL = [docURL URLByAppendingPathComponent:fileName];
    });
    return storeURL;
}
...
-(void)loadPersistentContainer {
        ...
        self.persistentContainer = [[NSPersistentContainer alloc] initWithName:@"AxeSync" managedObjectModel:model];
